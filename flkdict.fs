...
[META] HF-INDIRECT [PREVIOUS] CONSTANT HF-INDIRECT
[META] HF-TEMPORARY [PREVIOUS] CONSTANT HF-TEMPORARY
  ...
  WHILE 				\ ca u xt
    ...
    (swl-COMPARE) 0= IF 		\ ca u / r: xt
      ...
      DUP HF-VISIBLE AND IF 		\ ca u xt flags
        2SWAP 2DROP 			\ xt flagsA
        DUP HF-INDIRECT AND IF SWAP >DFA @ SWAP THEN
	HF-IMMEDIATE AND 
	IF 1 ELSE -1 THEN
	EXIT
      ELSE 				\ ca u xt flags
